...
<script>
    $('.lihat-gambar').click(function() {
        ...
        $.ajax({
            url: "<?= base_url() ?>dashboard/petugas/petugas/getBukuById",
            data: {
                "id": id,
            },
            dataType: "json",
    ...
    $(document).on('click', '.badge.badge-primary.edit', function() {
        const id = $(this).data("id");
        $('#formJenis').attr("action", "<?= base_url() ?>dashboard/petugas/petugas/updateJenis");
        $('.hidden.id').val(id);
        $('#formModal').html("Edit Data");
        console.log(id);
        $.ajax({
            url: "<?= base_url() ?>/dashboard/petugas/petugas/getJenisPerIdAjax",
            data: {
                ids: id,
            },
            type: "post",
    ...
    $(document).on('click', '#tombol_tambah_jenis', function() {
        $('#formJenis').attr("action", "<?= base_url() ?>dashboard/petugas/petugas/tambahJenis");
        $('.hidden.id').val("");
        $('#formModal').val("");
        $('#nama_jenis').val("");
